...
public class MazeCreator : MonoBehaviour
{
    ...
    int r; //乱数の値
    [SerializeField] GameObject wall,item,goal; //壁オブジェクト
    [SerializeField] GameObject maze;//迷路盤面
    GameObject mapelement;//壁のゲームオブジェクト
    ...
    void Start()
    {
        ...
        for (z = 0; z < max_z; z = z + 1) //フィールドの縦幅の分だけループする。
        {
            for (x = 0; x < max_x; x = x + 1) //フィールドの横幅の分だけループする。
            {
                if (field[z, x] == 0 && z % 5 == 0 && x % 5 ==0) //通路なら
                {
                    mapelement=Instantiate(item, new Vector3(2.0f * x, 1.0f, 2.0f * z), Quaternion.identity);//何も配置しない。
                }
                else if (field[z, x] == 1) //壁なら
                {
                    mapelement = Instantiate(wall, new Vector3(2.0f * x, 2.0f, 2.0f * z), Quaternion.identity); //壁を配置する。
                }
                else if(z==max_z - 1 && x==max_x - 2)
                {
                    mapelement = Instantiate(goal, new Vector3(2.0f * x, 2.0f, 2.0f * z), Quaternion.identity);
                }
                mapelement.transform.SetParent(maze.transform,false);
            }
        }
    }
}